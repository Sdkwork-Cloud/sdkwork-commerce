...
async fn list_admin_membership_plans(
    ...
    let rows = sqlx::query(
        ...
        ORDER BY p.rank ASC, p.plan_no ASC, b.sort_weight ASC NULLS LAST, b.id ASC
        "#,
    )
...
async fn load_admin_membership_plan(
    ...
    let rows = sqlx::query(
        ...
        ORDER BY b.sort_weight ASC NULLS LAST, b.id ASC
        "#,
    )
    .bind(plan_id)
    .fetch_all(pool)
...
async fn insert_order(
    ...
    sqlx::query(
        ...
        VALUES
            ($1, CAST($2 AS TEXT), CAST($3 AS TEXT), CAST($4 AS TEXT), $5, 'pending_payment', 'pending', 'unfulfilled', 'none', 'membership', 'CNY', $6, $7, $8, NULL, NULL, $9, $8)
        "#,
    )
...
async fn insert_order_item(
    ...
    sqlx::query(
        ...
        VALUES
            ($1, CAST($2 AS TEXT), $3, CAST($4 AS TEXT), $5, $6, 1, $7, $7, 'unfulfilled', 'none', $8)
        "#,
    )
...
async fn insert_payment(
    ...
    sqlx::query(
        ...
        VALUES
            ($1, CAST($2 AS TEXT), CAST($3 AS TEXT), CAST($4 AS TEXT), $5, $6, $7, $8, $9, 'CNY', $10, $11, $12, $13, $13)
        "#,
    )
    ...
    sqlx::query(
        ...
        VALUES
            ($1, CAST($2 AS TEXT), CAST($3 AS TEXT), CAST($4 AS TEXT), $5, $6, $7, $8, $9, $10, 'CNY', $11, $12, $13, NULL, $13)
        "#,
    )
...
async fn insert_membership(
    ...
    sqlx::query(
        ...
        VALUES
            ($1, CAST($2 AS TEXT), CAST($3 AS TEXT), $4, 'user', CAST($5 AS TEXT),
             CAST($5 AS TEXT), $6, $7, $8, $9, $10, $11, 'pending_activation', $12, $13,
             NULL, 0, $14, $15, $16, $16)
        "#,
    )
    ...
    sqlx::query(
        ...
        VALUES
            ($1, CAST($2 AS TEXT), CAST($3 AS TEXT), $1, $4, 'user',
             CAST($5 AS TEXT), $6, $7, $8, $9, 'pending_activation',
             $10, $11, $12, $13, $14, $14)
        "#,
    )
    .bind(&period_id)
...
async fn insert_entitlements(
    ...
    for (index, benefit) in plan.benefits.iter().enumerate() {
        ...
        sqlx::query(
            ...
            VALUES
                ($1, CAST($2 AS TEXT), CAST($3 AS TEXT), $1, $4, 'user', CAST($5 AS TEXT),
                 'membership_subscription', $6, 'membership_plan', $7, 'active', $8, $9,
                 $10, $11, $12, $12)
            "#,
        )
        .bind(&grant_id)
        ...
        sqlx::query(
            ...
            VALUES
                ($1, CAST($2 AS TEXT), CAST($3 AS TEXT), $1, $4, $5, $6,
                 'user', CAST($7 AS TEXT), 'credit', $8, $9, 'membership_grant',
                 'membership_subscription', $10, $11, $12, $13, $13)
            "#,
        )
        .bind(&ledger_id)
        .bind(command.subject.tenant_id)
...
async fn upsert_entitlement_account(
    ...
    let row = sqlx::query(
        ...
        VALUES
            ($1, CAST($2 AS TEXT), CAST($3 AS TEXT), $1, $4, 'user',
             CAST($5 AS TEXT), $6, '0', $6, 'active', $7, 0, $8, $8)
        ON CONFLICT(tenant_id, subject_type, subject_id, benefit_id) DO UPDATE SET
            total_granted = CAST((CAST(entitlement_account.total_granted AS BIGINT) + CAST(EXCLUDED.total_granted AS BIGINT)) AS TEXT),
            balance = CAST((CAST(entitlement_account.balance AS BIGINT) + CAST(EXCLUDED.balance AS BIGINT)) AS TEXT),
            status = 'active',
            expires_at = CASE
...
async fn consume_speed_up(
    ...
    let row = sqlx::query(
        ...
        WHERE a.tenant_id = CAST($1 AS TEXT)
          ...
          AND CAST(a.balance AS BIGINT) > 0
        ORDER BY a.expires_at DESC NULLS LAST, a.updated_at ASC NULLS LAST, a.created_at ASC NULLS LAST, a.id ASC
        LIMIT 1
        "#,
    )
    .bind(subject.tenant_id)
