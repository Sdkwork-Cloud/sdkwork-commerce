...
pub(crate) fn numeric_runtime_subject_from_extension(
    ...
    let organization_id = context
        .organization_id
        .as_deref()
        .ok_or_else(|| "authenticated runtime context organization_id is required".to_owned())?;
    Ok(NumericRuntimeSubject {
        tenant_id: parse_positive_context_i64(&context.tenant_id, "tenant_id")?,
        organization_id: parse_positive_context_i64(organization_id, "organization_id")?,
