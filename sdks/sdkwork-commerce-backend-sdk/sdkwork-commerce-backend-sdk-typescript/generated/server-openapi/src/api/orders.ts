...
export interface OrdersCancellationsListParams {
  status?: string;
  page?: number;
  pageSize?: number;
}
...
/** Orders cancellations list. */
  async list(params?: OrdersCancellationsListParams): Promise<CommerceApiResult> {
    const query = buildQueryString([
      { name: 'status', value: params?.status, style: 'form', explode: true, allowReserved: false },
      { name: 'page', value: params?.page, style: 'form', explode: true, allowReserved: false },
      { name: 'page_size', value: params?.pageSize, style: 'form', explode: true, allowReserved: false },
    ]);
    return this.client.get<CommerceApiResult>(appendQueryString(backendApiPath(`/orders/cancellations`), query));
  }
}
...
export class OrdersManagementApi {
  private client: HttpClient;
