...
/** Invoices mine list. */
  async list(params?: InvoicesMineListParams): Promise<CommerceApiResult> {
...
/** Invoices items list. */
  async list(invoiceId: string): Promise<CommerceApiResult> {
    return this.client.get<CommerceApiResult>(appApiPath(`/invoices/${serializePathParameter(invoiceId, { name: 'invoiceId', style: 'simple', explode: false })}/items`));
  }
}
